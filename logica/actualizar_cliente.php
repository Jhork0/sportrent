...
// Procesar datos para actualizar la tabla persona
...
if ($correo_nuevo !== null) {
    $verifica = "SELECT id_credencial FROM credencial WHERE usuario = ? AND id_credencial != ?";
    $stmtVerifica = $conn->prepare($verifica);
    $stmtVerifica->bind_param("ss", $correo_nuevo, $id_credencial);
...
if (!empty($updates)) {
    try {
        ...
        // Si se modificó el correo, también actualizamos credencial
        if ($correo_nuevo !== null) {
            // Obtener el id_credencial del cliente
            $queryCred = "SELECT u.id_credencial FROM usuario u WHERE u.cedula_persona = ?";
            ...
            if ($resultCred->num_rows > 0) {
                ...
                // Actualizar correo en la tabla credencial
        ...
        header("Location: ../vistas/home.php?mensaje=Perfil actualizado");
        exit();
    } catch (Exception $e) {
        echo "Error al actualizar: " . $e->getMessage();
    }
} else {
    header("Location: ../vistas/home.php?mensaje=No se realizaron cambios");
    exit();
}
?>